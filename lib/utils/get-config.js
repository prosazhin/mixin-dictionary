...
const checking = {
  ...
  themes: (value) => {
    ...
    if (typeof value !== 'object' || Array.isArray(value)) {
      redLog('themes must be an object with "light" and/or "dark" keys');
      return null;
    }
    ...
    if (value.light) {
      if (!Array.isArray(value.light) || !value.light.length) {
        redLog('themes.light must be a non-empty array of file paths');
      } else {
        result.light = value.light;
      }
    }

    if (value.dark) {
      if (!Array.isArray(value.dark) || !value.dark.length) {
        redLog('themes.dark must be a non-empty array of file paths');
      } else {
        result.dark = value.dark;
      }
    }
